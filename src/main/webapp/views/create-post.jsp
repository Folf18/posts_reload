...
<div class="row justify-content-center">
    <form class="col-md-4" action="/add-ads" method="post">
        <div class="form-group">
            Enter short summary: <input type="text" name="summary" value="${oldSummary}" placeholder="Summary e.g. 'Selling a cow'"
        ...
        <div class="form-group">
            ...
            <select class="form-control col-md-6" name="post_type_id">
                <c:forEach items="${postTypes}" var="postType">
                   <option value="${postType.id}" <c:if test="${oldPostTypeId eq postType.id}">selected="selected"</c:if>>${postType.name}</option>
                </c:forEach>
            </select>
            </br>
        </div>
